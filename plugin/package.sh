...
#   bash plugin/package.sh          -> dist/com.4xsdev.acs-edge.streamDeckPlugin        (Marketplace build, no Ko-fi key)
#   bash plugin/package.sh --kofi   -> dist/com.4xsdev.acs-edge-kofi.streamDeckPlugin   (GitHub build, adds the Ko-fi key)
# Elgato's guidelines forbid donation links inside plugins, so only the GitHub build carries the Ko-fi action.
set -euo pipefail
cd "$(dirname "$0")/.."
P=com.4xsdev.acs-edge.sdPlugin
OUT=com.4xsdev.acs-edge.streamDeckPlugin
rm -rf dist/$P && mkdir -p dist/$P
cp -R plugin/$P/. dist/$P/
if [[ "${1:-}" == "--kofi" ]]; then
  OUT=com.4xsdev.acs-edge-kofi.streamDeckPlugin
